...
def main(image_file, tess_args):
    ...
    # cv2.imwrite('/home/swha/improve/framework/test002/tmp/' + filename, image)
    # cv2.imwrite('/home/swha/improve/framework/test002/tmp/' + filename + '_cropped', cropped)
    ...
    if not tess_args:
        ...
        tessdata_dir = '/home/swha/improve/framework/models/'
        tess_args = ["--oem","1","--psm", "7", "-l", "kor-lstm", "--tessdata-dir", tessdata_dir]
    # txt = ocr_image(cropped, " ".join(tess_args))
    txt = ocr_image(image, " ".join(tess_args))
